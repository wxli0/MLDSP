function [AcNmb, Seq, numberOfClusters, clusterNames, pointsPerCluster,FNm] = readExterns(testingSet, maxClusSize)
    path = pwd;
    ...
    for i=1:numberOfClusters
        ...
        if(pts{i} <= szLimit)
            ...
            parfor j=1:pts{i}
                ...
                seqTemp{j} = Sequence;
                acTemp{j} = Header; 
                fnTemp{j} = sbName(1:length(sbName)-6);
            end            
        else
