function [hdl,vec] = f_pcoaPlot(result,sLabels,Y,yLabels,offset,fmt,wt)
% - plot results from a Principal Coordinates Analysis
%
% USAGE: [hdl,vec] = f_pcoaPlot(result,sLabels,Y,yLabels,offset,fmt,wt);
%
% result  = structure of results obtained from f_pcoa
%
...
% If only 1 PCoA axis, add 2nd jittered axis:
if (size(scores,2)==1)
   a      = min(scores) * 0.25;
   b      = max(scores) * 0.25;
...
% -----Create PCoA Plot:-----
figure('Name','PCoA: Principal Coordinates Analysis');
...
title('\bfPrincipal Coordinates Analysis');
...
if (~isempty(Y))
   ...
   % Correlation of original Y variables with each PCoA axis:
   ...
   if (wt<1)
      ...
      xText = 'Correlation with Axis I';
      yText = 'Correlation with Axis II';
   else
      xText = 'Axis I';
      yText = 'Axis II';
   end
