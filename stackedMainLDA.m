...
if (~strcmp(testSet,''))
    ...
    % construct Y3
    Y3 = zeros(length(alabels), length(unique(alabels)));
    ...
    for i=1:length(alabels)
        ...
        Y3(i,:) = -sqrt(labelCount(index)/totalCount);
    end

    % second round for yi == j
    for i = 1:length(alabels)
        for j = 1:length(labelCat)
            index = 0;
            for k=1:length(labelCat)
                if strcmp(labelCat{k},int2str(alabels(i)))
                    index = k;
            ...
            if index == j
                Y3(i,j) = sqrt(totalCount/labelCount(index))-sqrt(labelCount(index)/totalCount);
            end
        end
    end
