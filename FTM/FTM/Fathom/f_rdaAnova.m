function anova = f_rdaAnova(Y,X,iter,verb)
% - permutation-based (M)ANOVA via RDA
%
% USAGE: anova = f_rdaAnova(Y,X,iter,verb)
%
% Y     = matrix of response variables (column-wise)
% X     = matrix of integers specifying group membership for objects in Y
% iter  = # iterations for permutation test  (default = 0)
% verb  = optionally send results to display (default = 1)
%
% SEE ALSO: f_rda, f_rdaDB, f_npManova
...
% -----Notes:-----
% This function performs (M)ANOVA using RDA by re-coding the matrix of group
...
switch ncX
   case 1; % 1-Way ANOVA:
      ...
      A      = f_xMatrix(X,1);        % create ANOVA design matrix
      result = f_rda(Y,A,[0],iter,0); % 1-way ANOVA via RDA
      anova  = sub_format(result,1);  % format as an anova table


   case 2 % 2-way ANOVA:
      
      if (model == 22) % ORTHOGONAL:
         A         = f_xMatrix(X(:,1),1);        % factor 1
         B         = f_xMatrix(X(:,2),1);        % factor 2
         [null,AB] = f_xMatrix(X(:,1:2),1);      % interaction
         result.A  = f_rda(Y,A,[B AB],iter,0,0,0);
         result.B  = f_rda(Y,B,[A AB],iter,0,0,0);
         result.AB = f_rda(Y,AB,[A B],iter,0,0,0);
         anova     = sub_format(result,2);       % format as anova table
...
if (verb>0)
   ...
   fprintf('\n==================================================\n');
   fprintf('    Permutation-based (M)ANOVA via RDA:\n');
   fprintf('--------------------------------------------------\n');
   disp(resultCell);
   fprintf('\n %20s %10d \n',['# iterations = '],iter);
   fprintf('--------------------------------------------------\n');
...
% - format RDA results as an ANOVA table
switch fmt
   case 1 % 1-way ANOVA:
      anova(1).so = {'factor 1'};
      anova(2).so = {'residual'};
