...
tab1 = read.table(file = "/Users/wanxinli/Desktop/project.nosync/MLDSP-desktop/samples/rumen_mag_metadata.csv", sep = ",", header = TRUE)
...
png("paper_plots/rumen_GC.png", width = 324.18, height = 400)
...
png("paper_plots/rumen_contig.png", width = 324.18, height = 400)
...
qplot(tab1$contig_count, geom = "histogram", ylab = "Number of genomes (all)", xlab = "Contig count (max 1000 displayed)")
dev.off()

#genome size
png("paper_plots/rumen_genome.png", width = 324.18, height = 400)
...
qplot(tab1$genome_size, geom = "histogram", ylab = "Number of genomes (all)", xlab = "Genome size in basepairs (max 1.25 Gbp displayed)")
dev.off()
